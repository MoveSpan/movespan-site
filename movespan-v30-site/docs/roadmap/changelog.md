# Changelog

## July 2026
- Migrated from Netlify to Cloudflare Pages
- Added Sleep Reset (renamed from Sleep Preparation)
- Added 9 body zones (simplified names)
- Added AquaBreath as 8th practice
- Max 3 practices / 3 zones in onboarding
- New slogan: Move better · Feel brighter · Live longer
- SVG metronome button on Sleep Reset card
- Metronome accent OFF by default
